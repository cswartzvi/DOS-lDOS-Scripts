...
foreach my $ldos_file (glob($ldos_dir.'/intKS.dat*')) {
   open LDOSFILE, "<", $ldos_file or die " ERROR: Cannot open $ldos_file!!\n";

   #Find the correct EigFile
   (my $prod_step = $ldos_file) =~ s/.+intKS\.dat//;
   my $eig_file = $eig_dir.'/eig.dat'.$prod_step;
   
   #my $val = open EIGFILE, "<", $eig_file or die " ERROR: Cannot open $eig_file!!\n";
   if (-e $eig_file) {
      open EIGFILE, "<", $eig_file; 
      print "Opening Eig_file";
   }
   else {
      print " $prod_step File does not exist\n";
      next;
   }
